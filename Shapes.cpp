...
const int numGrassVertices = 36; //(6 faces)(2 triangles/face)(3 vertices/triangle)

point4 grassPoints [numGrassVertices];
point3 grassNormals[numGrassVertices];
point2 grassUV     [numGrassVertices];

// Vertices of a unit cube centered at origin, sides aligned with axes
point4 gvertices[8] = {
    point4( -0.5, -0.5,  0.1, 1.0 ),
    point4( -0.0,  1.0,  0.1, 1.0 ),
    point4(  0.5,  -0.5,  0.1, 1.0 ),
    point4(  0.0, -0.5,  0.1, 1.0 ),
    point4( -0.5, -0.5, -0.1, 1.0 ),
    point4( -0.0,  1.0, -0.1, 1.0 ),
    point4(  0.5,  -0.5, -0.1, 1.0 ),
    point4(  0.0, -0.5, -0.1, 1.0 )
...
//    to the vertices
int gIndex = 0;
void gquad( int a, int b, int c, int d, const point3& normal )
{
    grassPoints[gIndex] = gvertices[a]; grassNormals[gIndex] = normal; 
    grassUV[gIndex] = point2(0.0f, 1.0f); gIndex++;
    grassPoints[gIndex] = gvertices[b]; grassNormals[gIndex] = normal;
    grassUV[gIndex] = point2(0.0f, 0.0f); gIndex++;
    grassPoints[gIndex] = gvertices[c]; grassNormals[gIndex] = normal;
    grassUV[gIndex] = point2(1.0f, 0.0f); gIndex++;
    grassPoints[gIndex] = gvertices[a]; grassNormals[gIndex] = normal;
    grassUV[gIndex] = point2(0.0f, 1.0f); gIndex++;
    grassPoints[gIndex] = gvertices[c]; grassNormals[gIndex] = normal;
    grassUV[gIndex] = point2(1.0f, 0.0f); gIndex++;
    grassPoints[gIndex] = gvertices[d]; grassNormals[gIndex] = normal;
    grassUV[gIndex] = point2(1.0f, 1.0f); gIndex++;
}

// generate 12 triangles: 36 vertices, 36 normals, 36 texture coordinates
void colorgrass()
{
    gquad( 1, 0, 3, 2, point3( 0.0f,  0.0f,  1.0f) );
    gquad( 2, 3, 7, 6, point3( 1.0f,  0.0f,  0.0f) );
    gquad( 3, 0, 4, 7, point3( 0.0f, -1.0f,  0.0f) );
    gquad( 6, 5, 1, 2, point3( 0.0f,  1.0f,  0.0f) );
    gquad( 4, 5, 6, 7, point3( 0.0f,  0.0f, -1.0f) );
    gquad( 5, 4, 0, 1, point3(-1.0f,  0.0f,  0.0f) );
}

// initialization
void generateGrass(GLuint program, ShapeData* grassData)
{
    colorgrass();
    grassData->numVertices = numGrassVertices;

    // Create a vertex array object
    glGenVertexArrays( 1, &grassData->vao );
    glBindVertexArray( grassData->vao );

    // Set vertex attributes
    setVertexAttrib(program, 
        (float*)grassPoints,  sizeof(grassPoints), 
        (float*)grassNormals, sizeof(grassNormals),
        (float*)grassUV,      sizeof(grassUV));
}

//----------------------------------------------------------------------------
// Heart

const int numHeartVertices = 36; //(6 faces)(2 triangles/face)(3 vertices/triangle)

point4 heartPoints [numHeartVertices];
point3 heartNormals[numHeartVertices];
point2 heartUV     [numHeartVertices];

// Vertices of a unit cube centered at origin, sides aligned with axes
point4 hvertices[8] = {
    point4( -0.5, -0.2,  0.5, 1.0 ),
    point4(  0.1, -0.1,  0.5, 1.0 ),
    point4(  0.2,  0.5,  0.5, 1.0 ),
...
//    to the vertices
int hIndex = 0;
void hquad( int a, int b, int c, int d, const point3& normal )
{
    heartPoints[hIndex] = hvertices[a]; heartNormals[hIndex] = normal; 
    heartUV[hIndex] = point2(0.0f, 1.0f); hIndex++;
    heartPoints[hIndex] = hvertices[b]; heartNormals[hIndex] = normal;
    heartUV[hIndex] = point2(0.0f, 0.0f); hIndex++;
    heartPoints[hIndex] = hvertices[c]; heartNormals[hIndex] = normal;
    heartUV[hIndex] = point2(1.0f, 0.0f); hIndex++;
    heartPoints[hIndex] = hvertices[a]; heartNormals[hIndex] = normal;
    heartUV[hIndex] = point2(0.0f, 1.0f); hIndex++;
    heartPoints[hIndex] = hvertices[c]; heartNormals[hIndex] = normal;
    heartUV[hIndex] = point2(1.0f, 0.0f); hIndex++;
    heartPoints[hIndex] = hvertices[d]; heartNormals[hIndex] = normal;
    heartUV[hIndex] = point2(1.0f, 1.0f); hIndex++;
}

// generate 12 triangles: 36 vertices, 36 normals, 36 texture coordinates
void colorheart()
{
    hquad( 1, 0, 3, 2, point3( 0.0f,  0.0f,  1.0f) );
    hquad( 2, 3, 7, 6, point3( 1.0f,  0.0f,  0.0f) );
    hquad( 3, 0, 4, 7, point3( 0.0f, -1.0f,  0.0f) );
    hquad( 6, 5, 1, 2, point3( 0.0f,  1.0f,  0.0f) );
    hquad( 4, 5, 6, 7, point3( 0.0f,  0.0f, -1.0f) );
    hquad( 5, 4, 0, 1, point3(-1.0f,  0.0f,  0.0f) );
}

// initialization
void generateHeart(GLuint program, ShapeData* heartData)
{
    colorheart();
    heartData->numVertices = numHeartVertices;

    // Create a vertex array object
    glGenVertexArrays( 1, &heartData->vao );
    glBindVertexArray( heartData->vao );

    // Set vertex attributes
    setVertexAttrib(program, 
        (float*)heartPoints,  sizeof(heartPoints), 
        (float*)heartNormals, sizeof(heartNormals),
        (float*)heartUV,      sizeof(heartUV));
}
